...
=======
const SAMPLE_LETTERS = [
  { title: "To My Younger Self", text: "Stop waiting for permission to take up space. You belong in every room you walk into.", icon: "🌸" },
  { title: "To Women Who Feel Behind", text: "There is no timeline. Your path is not delayed — it's yours. Keep going.", icon: "💌" },
  { title: "To Women Healing", text: "Rest is not something you earn. You are allowed to slow down before you break.", icon: "🕊️" },
  { title: "To the Girl Doubting Herself", text: "Your voice matters. The world needs exactly what only you can offer. Say the thing. Take the seat.", icon: "✨" },
  { title: "To Women in Male-Dominated Spaces", text: "You are not too much. You are not too loud. You are exactly what that room has been missing.", icon: "💪" },
  { title: "To Women With PCOS", text: "Your body is not broken. It is working hard in ways most people will never understand. You deserve gentleness.", icon: "🌿" },
  { title: "To Women Healing from Loss", text: "Grief is not a sign of weakness. It is the proof of how deeply you loved. Take all the time you need.", icon: "🤍" },
  { title: "To My Future Self", text: "I hope by the time you read this, you've learned to rest without guilt. You deserve softness and joy.", icon: "🌙" },
  { title: "To Women Starting Over", text: "Starting over is not failure. It is the bravest thing you can do. Your next chapter is waiting.", icon: "🦋" },
]

export default function Home({ setPage, onSignOut, user, setSelectedCategory }) {
...
=======
      {/* SAMPLE LETTERS AS CATEGORIES */}
      <div style={{ padding: '4rem 3rem', background: 'white' }}>
        <h2 style={{ textAlign: 'center', color: '#3d1f35', marginBottom: '0.5rem', fontSize: '1.6rem' }}>
          From our community
        </h2>
        <p style={{ textAlign: 'center', color: '#a06080', fontSize: '0.9rem', marginBottom: '2rem' }}>
          Click a letter to browse that category
        </p>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '2rem', flexWrap: 'wrap' }}>
          {SAMPLE_LETTERS.map((letter) => (
            <div
              key={letter.icon}
              onClick={() => handleCategoryClick(letter.icon)}
              style={{
                background: '#fdf6f0', borderRadius: '16px',
                padding: '2.5rem 2rem', width: '280px',
                boxShadow: '0 4px 20px rgba(180,100,140,0.1)',
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = '0 8px 28px rgba(124,63,110,0.15)'
                e.currentTarget.style.border = '1px solid #c084a0'
              }}
              onMouseLeave={e => {
                e.currentTarget.style.transform = 'none'
                e.currentTarget.style.boxShadow = '0 4px 20px rgba(180,100,140,0.1)'
                e.currentTarget.style.border = '1px solid #f0dde8'
              }}
            >
>>>>>>> 7650e760d032e67a396b918c2d7a3be3f7bf90fe
